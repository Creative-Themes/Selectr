...
;(function(root, factory) {
	...
	var util = {
		escapeRegExp: function(str) {
			...
			return str && _reHasRegExpChar.test(str)
				? str.replace(_reRegExpChar, '\\$&')
				: str
		},
	...
	var build = function() {
		...
		if (this.el.multiple) {
			...
			this.selectedValues = this.config.defaultSelected
		...
		if (this.config.taggable) {
			...
			if (this.config.tagSeperators) {
				...
				for (
					var _nTagSeperatorStepCount = 0;
					_nTagSeperatorStepCount < this.tagSeperators.length;
					_nTagSeperatorStepCount++
				) {
					_aTempEscapedSeperators.push(
						util.escapeRegExp(
							this.tagSeperators[_nTagSeperatorStepCount]
						)
					)
				}
				this.tagSeperatorsRegex = new RegExp(
					_aTempEscapedSeperators.join('|'),
					'i'
				)
			} else {
	...
	var match = function(query, option) {
		...
		if (result) {
			return option.textContent.replace(
				result[0],
				"<span class='selectr-match'>" + result[0] + '</span>'
			)
		}
		return false
	}
	...
	Selectr.prototype.bindEvents = function() {
		...
		this.container.addEventListener('keydown', function(e) {
			...
			if (e.key === 'Enter' && that.selected === document.activeElement) {
				if (that.el.form && typeof that.el.form.submit !== 'undefined')
					that.el.form.submit()
			}
		...
		if (this.config.nativeKeyboard) {
			...
			this.selected.addEventListener('keydown', function(e) {
				...
				if (
					e.key === ' ' ||
					(!that.opened &&
						['Enter', 'ArrowUp', 'ArrowDown'].indexOf(e.key) > -1)
				) {
					that.toggle()
					e.preventDefault()
					e.stopPropagation()
				...
				if (
					e.key.length <= 2 &&
					String[
						String.fromCodePoint ? 'fromCodePoint' : 'fromCharCode'
					](
						e.key[
							String.codePointAt ? 'codePointAt' : 'charCodeAt'
						](0)
					) === e.key
				) {
		...
		if (this.config.taggable) {
			this.input.addEventListener('keyup', function(e) {
				...
				if (that.config.taggable && this.value.length) {
					...
					if (
						_sVal.length &&
						(e.which === 13 || that.tagSeperatorsRegex.test(_sVal))
					) {
						var _sGrabbedTagValue = _sVal.replace(
							that.tagSeperatorsRegex,
							''
						)
	 ...
	 * @param {bool} reset
	 */
	Selectr.prototype.setSelected = function(reset, skipEventGeneration) {
		...
		if (!this.config.data && !this.el.multiple && this.el.options.length) {
			...
			if (this.selectedIndex > -1) {
				this.select(this.selectedIndex, skipEventGeneration)
			}
		}
		...
		util.each(
			...
			function(i, option) {
				if (option.selected && option.defaultSelected) {
					this.select(option.idx, skipEventGeneration)
				}
			},
			this
		)
		...
		if (this.config.data) {
			if (
				...
				this.select(0, skipEventGeneration)
			}
	 ...
	 * @return {void}
	 */
	Selectr.prototype.select = function(index, skipEventGeneration) {
		var item = this.items[index],
		...
		if (this.el.multiple) {
			...
			if (
				...
				this.setMessage(
					this.config.messages.maxSelections.replace(
						'{max}',
						this.config.maxSelections
					),
					true
			...
			this.label.innerHTML = this.customSelected
	...
	Selectr.prototype.setValue = function(value) {
		...
		util.each(
			...
			function(i, option) {
				if (
					(isArray && value.indexOf(option.value) > -1) ||
					option.value === value
				) {
	...
	Selectr.prototype.add = function(data, checkDuplicate) {
		if (data) {
			...
			if (Array.isArray(data)) {
				...
				option.idx =
					this.options.length > 0 ? this.options.length - 1 : 0
		...
		if (string.length > 0) {
			...
			if (live) {
				...
				if (!f.childElementCount) {
					...
					var firstEl = f.querySelector(
						'.selectr-option:not(.excluded)'
					)
